...
describe('getQuickstartFromFilename', () => {
    test('returns the quickstart an alert belongs to', () => {
      expect(getQuickstartFromFilename('quickstarts/python/aiohttp/alerts/ApdexScore.yml')).toEqual('aiohttp');
    })

    test('returns the quickstart a dashboard belongs to', () => {
      expect(getQuickstartFromFilename('quickstarts/python/pysqlite/dashboards/python.json')).toEqual('pysqlite');
    })

    test('returns the quickstart a logo belongs to', () => {
        expect(getQuickstartFromFilename('quickstarts/python/pysqlite/logo.svg')).toEqual('pysqlite');
      })

    test('returns a list of unique quickstarts', () => {
      expect(mockFilenames.map(getQuickstartFromFilename)).toEqual(expectedUniqueQuickstartDirectories);
    })
})