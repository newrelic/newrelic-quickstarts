...
|dataSourceIds|no|| A list of data sources |
...
- Each image file must be less than `4MB` in size, *please* optimize your screenshots for the web
...
|install            | yes     |       | The primary installation method. See [Install fields](#install-fields) for examples on how to use the `destination` field. *Note*: Data sources do not support the `targetedInstall` mode |
